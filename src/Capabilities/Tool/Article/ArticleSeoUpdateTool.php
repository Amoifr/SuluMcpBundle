...
class ArticleSeoUpdateTool
{
     ...
     */
    #[McpTool(
        name: 'sulu_article_seo_update',
        description: 'Update SEO metadata on an article draft. Only pass the fields you want to change — others are preserved. Re-publish via sulu_article_publish to make changes live.',
    )]
    public function updateArticleSeo(
        string $uuid,
        string $locale,
        ?string $title = null,
        ...
        try {
            ...
            return [
                'error' => \sprintf('Failed to update SEO for article %s: %s', $uuid, $e->getMessage()),
                'hint' => 'Verify the UUID exists (use sulu_article_seo_get).',
            ];
        }
    }
}
